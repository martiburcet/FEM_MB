...
f=fopen(fullfile('paraview','3D10_Paraview.vtk'),'w');
...
fprintf(f,'%s %8i %8i\n','CELLS', length(T) , length(T)*11);
for i1=1:length(T)     
  fprintf(f,'%4i  %10i  %10i %10i  %10i  %10i %10i %10i  %10i  %10i %10i\n',10,T(i1,2)-1,T(i1,3)-1,T(i1,4)-1,T(i1,5)-1,T(i1,6)-1,T(i1,7)-1,T(i1,8)-1,T(i1,9)-1,T(i1,10)-1,T(i1,11)-1);
end
fprintf(f,'\n');
fprintf(f,'%s %8i\n','CELL_TYPES', length(T));
for i1=1:length(T)
